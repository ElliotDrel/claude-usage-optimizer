...
### Timezone display
- **D-09 (revised 2026-04-23 per cross-AI review):** Fire times are displayed in the browser's local timezone by default. `app_meta.user_timezone` stores a valid **IANA timezone name** (e.g. `America/Los_Angeles`) — the scheduler's peak-detector requires IANA format. On page load, derive the UTC offset implied by the stored IANA name using `Intl.DateTimeFormat` and compare it to the browser's UTC offset (`new Date().getTimezoneOffset() / -60`). If they differ, show a **persistent** (non-blocking) notification banner: "Your scheduler timezone differs from your browser. [Dismiss]". Clicking "Dismiss" hides it for the session. The banner does NOT auto-update `app_meta` — user must manually set the correct IANA timezone via the Overrides panel. Do NOT write a raw UTC offset integer into `user_timezone` — that would break the scheduler's IANA validation.
